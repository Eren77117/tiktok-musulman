...
interface Notification {
  ...
  is_read: boolean;
  created_at: string;
  data?: { post_id?: string; user_id?: string; comment_id?: string };
}
...
export default function NotificationsScreen() {
  ...
  const markRead = (notif: Notification) => {
    ...
    qc.setQueryData(['notifications'], (old: any) => old ? {
      ...old, items: old.items.map((n: Notification) => n.id === notif.id ? { ...n, is_read: true } : n),
    } : old);
    ...
    if (postId && ['LIKE', 'COMMENT', 'SAVE', 'MENTION'].includes(notif.type)) {
      nav.navigate('VideoPlayer', { postId });
    } else if (userId && notif.type === 'FOLLOW') {
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              <Bell size={40} color={theme.textSubtle} strokeWidth={1.5} />
              <Text style={[styles.emptyTitle, { color: theme.text }]}>Aucune notification</Text>
              <Text style={[styles.emptySub, { color: theme.textMuted }]}>Tes notifications apparaîtront ici</Text>
            </View>
          }
        />
      )}
